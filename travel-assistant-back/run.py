...
dataset = load_data("tourist_places_sri_lanka.csv")
...
def recommend_trip():
    ...
    filtered_data = dataset[
        ...
        (
            ...
            (dataset['Categories'].apply(
                lambda x: any(cat.strip() in x for cat in categories)
            ))
        )
    ]
    ...
    data = request.json
    user_location = data['location']  # Required
    days = data['days']  # Required
    budget = data['budget']  # Required
    categories = data.get('categories', [])  # Optional, defaults to an empty list
    ...
    filtered_data = dataset[
        (dataset['Cost'] <= budget) &  # Check within budget
        (
            (not categories) |  # If categories is empty, include all places
            (dataset['Categories'].apply(
                lambda x: any(cat.strip() in x for cat in categories)
            ))
        )
    ...
    for rec in recommendations:
        ...
        for sub_place in rec.get('Sub Places', "").split(", "):
            sub_place_data = sub_places_coordinates.get(sub_place, {})
            if sub_place_data:
                sub_places_details.append({
                    "name": sub_place,
                    "latitude": sub_place_data.get("latitude"),
                    "longitude": sub_place_data.get("longitude")
                })
    ...
    for rec in recommendations:
        sub_places_details = []
        for sub_place in rec['Sub Places'].split(", "):
            sub_place_data = sub_places_coordinates.get(sub_place, {})
            if sub_place_data:
                sub_places_details.append({
                    "name": sub_place,
